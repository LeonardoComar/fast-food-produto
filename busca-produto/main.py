...
def process_message(message, queue_name):
    ...
    try:
        if queue_name == "produto-atualizacao":
            catalogo_data = CatalogoProduto.model_validate_json(data)
            ...
            preco_data = Preco.model_validate_json(data)
            stmt = insert(produto).values(
                sku=preco_data.sku,
            ...
            estoque_data = Estoque.model_validate_json(data)
            stmt = insert(produto).values(
...
@app.get("/produtos-disponiveis")
def obter_produtos_com_estoque():

  query = select(produto).where(produto.c.qtd > 0)
  
  with engine.connect() as connection:
    try:
      result = connection.execute(query).fetchall()
      if result is None or len(result) == 0:
        return {"detail": "Nenhum produto encontrado com estoque"}
